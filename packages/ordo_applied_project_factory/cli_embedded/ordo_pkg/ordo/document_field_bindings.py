...
def _node_edges(node: dict[str, Any]) -> set[str]:
    edges = {value["next"] for value in _walk_values(node.get("on_answer", {})) if isinstance(value, dict) and isinstance(value.get("next"), str)}
...
def _producers(node: dict[str, Any]) -> dict[str, dict[str, Any]]:
    ...
    for value in _walk_values(node.get("on_answer", {})):
        ...
        for field in updates:
            found[field] = {"node_id": node.get("id"), "collection_mode": node.get("collection_mode", "analyst_answer"), "required": field in set(node.get("required_fields", []) or []) or field in set((node.get("node_context") or {}).get("required_state", []) or []) or bool((node.get("field_contracts") or {}).get(field, {}).get("required")), "location": f"nodes[{node.get('id')}].on_answer.update_state.{field}"}
    return found
...
def validate_document_field_bindings(package: str | Path, bindings: str | Path) -> dict[str, Any]:
    ...
    documents = _load(Path(bindings)).get("documents", [])
    ...
    errors, warnings, documents_report = [], [], []
    ...
    for index, document in enumerate(documents if isinstance(documents, list) else []):
        ...
        report = {"document_id": document_id, "materialization_node": target, "paths": paths, "fields": []}
        if not paths:
        ...
        for item in document.get("required_fields", document.get("fields", [])) or []:
            spec = item if isinstance(item, dict) else {"field": item, "required": True}
            field = spec.get("field", spec.get("name"))
            if not field or not spec.get("required", True):
                continue
            preceding = [p for p in producers.get(field, []) if any(p["node_id"] in path[:-1] for path in paths)]
            field_report = {"field": field, "required": True, "producers": preceding, "status": "ok"}
            if field not in state_schema:
                field_report["status"] = "error"; errors.append({"code": "ORDO-DOC-FIELD-002", "document": document_id, "field": field, "message": "required document field is absent from state.schema", "location": str(source_file)})
            elif not preceding:
                field_report["status"] = "error"; errors.append({"code": "ORDO-DOC-FIELD-003", "document": document_id, "field": field, "message": "required document field has no producer before materialization on an applicable path", "location": str(target)})
            elif not all(any(p["node_id"] in path[:-1] for p in preceding) for path in paths):
                field_report["status"] = "error"; errors.append({"code": "ORDO-DOC-FIELD-004", "document": document_id, "field": field, "message": "required document field is not produced on every applicable path", "location": str(target)})
            elif not any(p.get("required") for p in preceding):
                field_report["status"] = "warning"; warnings.append({"code": "ORDO-DOC-FIELD-005", "document": document_id, "field": field, "message": "producer exists but collection is not marked required or gated upstream", "location": str(preceding[0].get("location"))})
            report["fields"].append(field_report)
        documents_report.append(report)
    return {"status": "failed" if errors else "passed_with_warnings" if warnings else "passed", "source": str(source_file), "bindings": str(bindings), "errors": errors, "warnings": warnings, "documents": documents_report}
