# Prompt: Build an All-Test-Cases Current-State-Only Ordo Evidence Transfer Archive

You are preparing an Ordo empirical evidence archive for transfer and integration.

Your task is to create a **current-state-only evidence snapshot covering every test case present in the supplied source evidence archive**.

The output must not be limited to the newest, most recently modified, most recently executed, or currently active test case. It must preserve the current authoritative state of **all test cases that belong to the evidence base**.

For every retained test case and every implementation condition within that test case, the output archive must contain only the latest accepted, approved, and currently authoritative evidence artifacts. Historical versions, superseded runs, intermediate packages, rejected variants, and duplicated version history must not be included.

A test case may be excluded only when the authoritative evidence catalog explicitly marks it as removed, deprecated, invalidated, or outside the declared transfer scope. Absence of recent activity is not a valid reason for exclusion.

## 1. Primary objective

Create a compact transfer archive that represents the **current accepted state of the complete evidence base across all retained test cases**.
...
## 1.1 Mandatory all-test-case coverage

Before selecting files, build an authoritative inventory of all test cases represented in the source evidence base.

The inventory must be derived from the current evidence catalog, case registry, manifests, run indexes, score ledgers, acceptance records, and explicit current-state declarations.

For every discovered test case:

- determine whether it is currently retained, deprecated, invalidated, removed, or out of scope;
- include every test case whose current lifecycle state requires retention;
- select the latest authoritative playbook or instruction package for each implementation condition;
- retain the latest accepted run for each required scenario, model, condition, or variant;
- retain current scorecards, audits, findings, claims, admissions, schemas, profiles, and bindings required to interpret those runs;
- preserve the exact older package version when a retained accepted run is bound to it;
- exclude prior versions and superseded attempts.

The archive must not silently collapse multiple test cases into one representative case.

The archive must not contain only:

- the newest test case;
- the last test case processed;
- the test case with the newest filesystem timestamp;
- the test case with the highest identifier;
- the test case mentioned in the latest README;
- the test case selected by a previous partial export.

Create a machine-readable test-case coverage matrix showing every discovered test case and its final disposition.

## 2. Core retention rule

For every logical artifact family **within every retained test case and implementation condition**, retain only the latest authoritative item.

Selection is performed independently per test case. An artifact selected for one test case must not replace or suppress the current artifact of another test case.
...
## 2.1 Cross-test-case retention invariant

The archive must preserve **all retained test cases**, not only one globally latest test case.

For each test case, independently retain:

- its latest authoritative case definition;
- the latest approved playbook or instruction package for every current implementation condition;
- the latest accepted run for every required scenario/model/condition;
- the exact run-bound package when it differs from the latest available package;
- the latest applicable evaluation profile, scorecard, audit, findings, claims, and provenance.

Historical versions inside each test case are excluded, but the test case itself remains present.
...
For each retained test case, and for each required test condition, scenario, model, or implementation variant within that test case:
...
For each retained test case and each implementation condition within it, retain only:
...
├── SHA256SUMS.txt
├── schemas/
├── TEST_CASE_COVERAGE_MATRIX.json
├── cases/
│   └── <test_case_id>/
├── accepted_runs/
│   └── <test_case_id>/
...
- selection policy;
- all test cases discovered in the source evidence base;
- retained test cases;
- excluded/deprecated/invalidated test cases and the authoritative reason for each disposition;
- per-test-case current package and accepted-run coverage;
...
## 10.1 Test-case coverage matrix

`TEST_CASE_COVERAGE_MATRIX.json` must contain one record for every test case discovered in the source evidence base.
...
- authoritative inclusion or exclusion reason;
- implementation conditions present;
- latest available playbook/package per condition;
- accepted-run-bound package per retained run;
- retained accepted runs;
- retained scorecards and audits;
...
=
retained_test_cases
+ authoritatively_excluded_test_cases
```

A missing disposition for any discovered test case is a release blocker.
...
- test-case inventory reconciliation;
- test-case coverage matrix completeness;
- no retained test case omitted merely because it is not the latest test case;
- accepted-run count reconciliation per test case and globally;
...
- any discovered test case lacks an authoritative inclusion or exclusion disposition;
- a retained test case has no current package/run binding where such evidence is required;
- the archive contains only the latest test case instead of all retained test cases.
...
- discovered test case count;
- retained test case count;
- authoritatively excluded test case count;
- per-test-case retained package and run summary;
...
```text
This archive is an all-test-cases current-state-only evidence snapshot.
It contains the current authoritative state of every retained test case in the evidence base.
...
Preserve the evidence needed to prove the current state of every retained test case, but do not preserve the complete path used to reach it.

“All test cases” and “current-state-only” must be applied together:

- breadth across all retained test cases;
- depth limited to the latest authoritative state of each test case;
- no historical version chains unless minimally required by a current bounded claim.
