...
## alpha.13 execution inheritance

Before final source generation, classify each executable node/gate responsibility as:
- `deterministic`
- `model_judgment`
- `human_authority`

Generated runnable packages must also materialize:

```text
PLAYBOOK_LAWS.md
verification_profile.json
verification/EXECUTION_RESPONSIBILITY_MAP.json
verification/INVARIANT_REGISTER.json
tools/<domain-specific deterministic validators, when required>
```
...
Deterministic helpers execute or validate mechanically resolvable work.
AI performs semantic judgment only where deterministic resolution is insufficient.
Humans own consequential content/approval decisions.
```

A generated package is not PRE_EDITOR-ready until `execution_responsibility_map` passes.


### Deterministic runtime binding rule

When a runtime responsibility is implemented by a deterministic helper, the generated source must expose that
relationship using canonical Ordo execution surfaces where applicable, including `node_context.allowed_tools`
and explicit evidence/state contracts. Do not leave a mechanically implemented responsibility described only
in model prose.

This follows the canonical Hybrid Execution model:
