...
## M63+ APF rc.1 integration backlog

- **M63.1 — APF rc.1 Package Import / Metadata Sync**: requires actual `v0.1.0-rc.1` APF archive as input; replaces historical alpha.14 import.
- **M63.2 — APF RC Validation Profile / Known Limitations**: formalizes parent-compatible validation vs APF-local validation.
- **M63.3 — APF RC Language Pattern Classification Update**: updates the pattern matrix for rc.1 without promoting concepts to core IR.
- **Future IR candidates**: `FLOW.JOIN`, `SHARED.TAIL.REFERENCE`.
- **Future parent CLI candidate**: `validate-factory-output`, only after APF-local semantics stabilize.
