...
## M61.2 update — Visual Graph Generator included

M61.2 imports the Visual Graph Generator under:

```text
utilities/ordo_visual_graph_generator/
```

The imported utility remains a read-only visual renderer. It is included beside PathWalk as a companion utility, not as Ordo runtime core. Its first supported usage is:

```text
source/program.ordo.yaml
  → Visual Graph Generator: Mermaid/SVG/PNG graph views and annotation overlays
  → PathWalk: terminal paths, testcase artifacts, review cards
```

M61.2 does not merge Visual Graph Generator into PathWalk, does not add runtime execution, and does not change scoring/calibration behavior.
