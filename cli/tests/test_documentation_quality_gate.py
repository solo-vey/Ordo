...
def test_vibe_arf_release_pointer_and_build_contract_are_current() -> None:
    pointer = load_json(ROOT / "manifests/VIBE_ARF_CURRENT.json")
    package = ROOT / "packages/vibe_arf"
    version = (package / "VIBE_ARF_VERSION").read_text(encoding="utf-8").strip()
    builder = (ROOT / "tools/build_vibe_arf.py").read_text(encoding="utf-8")

    assert pointer["schema_version"] == "ordo.vibe_arf_release_pointer.v1"
    assert pointer["version"] == version == "0.1.3"
    assert pointer["release_tag"] == f"vibe-arf-v{version}"
    assert pointer["recommended_profile"] == "MODEL_RUN"
    assert set(pointer["profiles"]) == {"MODEL_RUN", "CLI_RUN", "EDIT"}
    assert all(f"VIBE_ARF_{version}_" in url for url in pointer["profiles"].values())
    assert "legacy_arf_pointer" not in pointer
    assert "for profile in (\"EDIT\", \"CLI_RUN\", \"MODEL_RUN\")" in builder
    assert "build_production_evidence.py" in builder
