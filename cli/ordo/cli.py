...
from .prompt_compiler import compile_prompt_only, validate_prompt_compilation, assess_runtime_route
from .delivery_target import recommend_delivery_target, record_delivery_target_decision
...
from .manual_run_journey import validate_journey
...
def cmd_compile_prompt(args: argparse.Namespace) -> int:
    root, manifest, source, tests = load_package(args.package)
    source_path = root / manifest.get("source", "source/program.ordo.yaml")
    out_dir = Path(args.out).resolve() if args.out else root / "compiled" / "prompt_only"
    evidence = None
    if args.evidence:
        evidence = json.loads(Path(args.evidence).read_text(encoding="utf-8"))
    result = compile_prompt_only(source_path, out_dir, playbook_version=str(manifest.get("version", "unresolved")), language_version=str(manifest.get("ordo_version", "unresolved")), framework_version=args.framework_version, model_profile=args.model_profile, evidence_basis=evidence)
    validation = validate_prompt_compilation(out_dir, source_path=source_path)
    write_json(root / "reports" / "prompt_compilation_report.json", {"status": validation["status"], "manifest": result, "validation": validation})
    print(f"compile-prompt: {validation['status']} ({out_dir})")
...
def cmd_recommend_delivery_target(args: argparse.Namespace) -> int:
    assessment = json.loads(Path(args.assessment).read_text(encoding="utf-8"))
    policy = json.loads(Path(args.policy).read_text(encoding="utf-8")) if args.policy else None
    result = recommend_delivery_target(assessment, policy)
    if args.out:
        write_json(Path(args.out), result)
    print(json.dumps(result, indent=2))
    return 0
...
def cmd_record_delivery_target(args: argparse.Namespace) -> int:
    recommendation = json.loads(Path(args.recommendation).read_text(encoding="utf-8"))
    result = record_delivery_target_decision(recommendation, args.target, analyst_id=args.analyst_id, rationale=args.rationale or "")
    if args.out:
        write_json(Path(args.out), result)
    print(json.dumps(result, indent=2))
...
def cmd_validate_journey(args: argparse.Namespace) -> int:
    report = validate_journey(args.package, journey_path=args.journey)
    print(f"validate-journey: {report['status']} events={report.get('event_count', 0)}")
    return 0 if report["status"] == "passed" else 1
...
def cmd_package(args: argparse.Namespace) -> int:
    report = build_package_profile(
        ...
        ci_evidence=getattr(args, "ci_evidence", None),
    )
...
def cmd_template_validate(args: argparse.Namespace) -> int:
    report = validate_template_contract(args.contract)
    out = Path(args.out).resolve() if args.out else Path(args.contract).resolve().parent / "template_validation_report.json"
    write_json(out, report)
    print(f"template validate: {report['status']} ({out})")
    if report.get("issues"):
...
def cmd_template_registry_check(args: argparse.Namespace) -> int:
    report = validate_template_registry(args.registry)
    out = Path(args.out).resolve() if args.out else Path(args.registry).resolve().parent / "template_registry_consistency_report.json"
    write_json(out, report)
    print(f"template registry check: {report['status']} ({out})")
    if report.get("issues"):
        for issue in report["issues"][:5]:
            print(f"{issue.get('code')}: {issue.get('message')}", file=sys.stderr)
    return 0 if report["status"] == "passed" else 1
...
def cmd_template_render(args: argparse.Namespace) -> int:
    report = render_template(args.contract, args.input, args.output_dir)
    out = Path(args.report).resolve() if args.report else Path(args.output_dir).resolve() / "render_report.json"
    write_json(out, report)
    print(f"template render: {report['status']} mode={report.get('mode')} ({out})")
    if report.get("issues"):
        for issue in report["issues"][:5]:
            print(f"{issue.get('code')}: {issue.get('message')}", file=sys.stderr)
    return 0 if report["status"] == "passed" else 1
...
def cmd_template_review(args: argparse.Namespace) -> int:
    report = review_template_artifact(
        args.contract,
        args.artifact,
        render_evidence_path=getattr(args, "render_evidence", None),
        reviewer=getattr(args, "reviewer", "ordo-template-review-engine"),
    )
    out = Path(args.out).resolve() if args.out else Path(args.artifact).resolve().parent / "template_review_evidence.json"
    write_json(out, report)
    print(f"template review: {report['status']} decision={report.get('decision')} ({out})")
...
def cmd_template_diff(args: argparse.Namespace) -> int:
    report = diff_template_versions(args.old_contract, args.new_contract)
    out = Path(args.out).resolve() if args.out else Path(args.new_contract).resolve().parent / "template_version_diff.json"
    write_json(out, report)
    print(f"template diff: {report['status']} decision={report.get('decision')} ({out})")
    for finding in report.get("findings", [])[:5]:
        print(f"{finding.get('code')}: {finding.get('message')}", file=sys.stderr)
    return 0 if report["status"] == "passed" else 1
...
def build_parser() -> argparse.ArgumentParser:
    ...
    p = sub.add_parser("compile-prompt", help="Compile an Ordo playbook to a prompt-only instruction package")
    p.add_argument("package")
    p.add_argument("--out")
    p.add_argument("--framework-version", default="unresolved")
    p.add_argument("--model-profile", default="provider-neutral")
    p.add_argument("--evidence", help="Optional JSON evidence-basis record")
    p.set_defaults(func=cmd_compile_prompt)

    p = sub.add_parser("validate-prompt", help="Validate a prompt-only compilation and source binding")
    ...
    p.add_argument("--out")
    p.set_defaults(func=cmd_route_runtime)


    p = sub.add_parser("recommend-delivery-target", help="Recommend engine_runtime, prompt_only, or both after playbook validation")
    p.add_argument("--assessment", required=True)
    p.add_argument("--policy")
    p.add_argument("--out")
    p.set_defaults(func=cmd_recommend_delivery_target)

    p = sub.add_parser("record-delivery-target", help="Record the analyst's final ARF delivery target decision")
    p.add_argument("--recommendation", required=True)
    p.add_argument("--target", choices=["engine_runtime", "prompt_only", "both"], required=True)
    p.add_argument("--analyst-id", default="not_recorded")
    p.add_argument("--rationale")
    p.add_argument("--out")
    p.set_defaults(func=cmd_record_delivery_target)
    ...
    p.add_argument("package")
    p.add_argument("--journey", help="Optional journey YAML path")
    p.set_defaults(func=cmd_validate_journey)
    ...
    p = sub.add_parser("template", help="Validate, render, review, diff, and test reusable templates")
    template_sub = p.add_subparsers(dest="template_command", required=True)
    tp = template_sub.add_parser("validate", help="Validate one generic template contract")
    tp.add_argument("contract", help="YAML or JSON template contract")
    tp.add_argument("--out", help="Optional validation report path")
    tp.set_defaults(func=cmd_template_validate)
    tr = template_sub.add_parser("registry-check", help="Validate a Template Registry and all referenced contracts")
    tr.add_argument("registry", help="YAML or JSON Template Registry")
    tr.add_argument("--out", help="Optional consistency report path")
    tr.set_defaults(func=cmd_template_registry_check)
    trender = template_sub.add_parser("render", help="Render a template or create a controlled model-render job")
    trender.add_argument("contract", help="YAML or JSON template contract")
    trender.add_argument("--input", required=True, help="YAML or JSON render input")
    trender.add_argument("--output-dir", required=True, help="Directory for artifact/job/evidence outputs")
    trender.add_argument("--report", help="Optional CLI report path")
    trender.set_defaults(func=cmd_template_render)
    treview = template_sub.add_parser("review", help="Review a rendered artifact and emit structured evidence")
    treview.add_argument("contract", help="YAML or JSON template contract")
    treview.add_argument("--artifact", required=True, help="Rendered artifact to review")
    treview.add_argument("--render-evidence", help="Optional render_evidence.json; required by strict profiles")
    treview.add_argument("--reviewer", default="ordo-template-review-engine", help="Reviewer identity recorded in evidence")
    treview.add_argument("--out", help="Optional review evidence path")
    treview.set_defaults(func=cmd_template_review)
    tdiff = template_sub.add_parser("diff", help="Compare template contract versions and enforce breaking-change policy")
    tdiff.add_argument("old_contract", help="Previous template contract")
    ...
    p.add_argument("package")
    p.add_argument("--out")
    p.add_argument("--profile", choices=["dev", "runtime", "evidence", "prompt_only"], default="dev", help="Package build profile")
    ...
    p.add_argument("--ci-evidence", help="Path to current-tree green CI evidence; required for dev/runtime packaging")
