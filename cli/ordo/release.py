...
def validate_release(
    ...
    *,
    ...
    # Mandatory green release gate: do not create an archive while any blocking
    # validation issue exists. Packaging is a consequence of a green tree, not
    # a step that runs before the final decision.
    ...
    if archive_path.exists():
        archive_path.unlink()
    if errors:
        record_step("package", "blocked", None, {"reason": "mandatory green release gate failed", "blocking_errors": len(errors)})
    else:
    ...
    report = {
        ...
        "steps": steps,
        "release_archive": str(archive_path) if archive_path.exists() else None,
        "summary": {
            "errors": len(errors),
    ...
    # M14 release provenance is generated only for a successfully created archive.
    if archive_path.exists():
        ...
        provenance_validation = {"status": "blocked", "summary": {"reason": "release archive not created"}}
        record_step("build-provenance", "blocked", None, provenance_validation.get("summary"))
        record_step("validate-provenance", "blocked", None, provenance_validation.get("summary"))
    report["steps"] = steps
    report["provenance"] = {
        "path": "reports/release_provenance.json" if archive_path.exists() else None,
        "validation_report": "reports/release_provenance_validation_report.json" if archive_path.exists() else None,
        "status": provenance_validation.get("status"),
    }
    if archive_path.exists() and provenance_validation.get("status") != "passed":
    ...
    # M87.7: persist the build identity only after every release artifact
    # (archive, provenance, summary) exists, so the identity hashes the final
    # validated tree; then bind the report to it.
    build_identity = write_build_identity(root, reports_dir / "build_identity.json")
