...
## M68.4 implementation alignment

M68.4 implements the preferred direction from this design as an optional `repo-check --clean` mode. The implementation is intentionally policy-gated: without a repo hygiene policy, applied packages discovered under `packages/` are reported as delegated and are not enforced.

The implementation adds the `repo_package_hygiene` report section to existing repo-check output only when `--clean` is supplied. Default `repo-check` behavior remains unchanged.
