...
See `docs/EXTERNAL_ARCHIVES.md` for the historical stable path and `docs/PROJECT_TRACKING.md` for current work.
...
5. Fail fast with an actionable error if `utilities/ordo_pathwalk/` or `cli/ordo/` cannot be found.
...
```bash
PYTHONPATH=cli:. python3 -m utilities.ordo_pathwalk.cli init-tree \
  --depth 5 --branching 2 4 --seed 42 --out /tmp/pathwalk_tree --force

PYTHONPATH=cli:. python3 -m utilities.ordo_pathwalk.cli gen-scenarios \
  --tree /tmp/pathwalk_tree --count 10 --seed 100 --out /tmp/pathwalk_scenarios

PYTHONPATH=cli:. python3 -m utilities.ordo_pathwalk.cli run \
...
PYTHONPATH=cli:. python3 -m utilities.ordo_pathwalk.cli score \
  --scenario /tmp/pathwalk_scenarios/scenario_000.json \
...
PYTHONPATH=cli:. python3 -m utilities.ordo_pathwalk.cli aggregate \
...
```bash
PYTHONPATH=cli:. pytest utilities/ordo_pathwalk/tests -q
```
...
Sample output is kept in `utilities/ordo_pathwalk/examples/m60_5_matrix_smoke/`.
...
PYTHONPATH=cli:. python3 -m utilities.ordo_pathwalk.cli dry-run-job \
...
```bash
PYTHONPATH=cli:. python3 -m utilities.ordo_pathwalk.cli dry-run-plan   --out runs/m60_6_calibration_prep   --scenario-count 20   --runtime-view json   --runtime-view ordo-code   --runtime-view json,ordo-code   --force

# Execute generated jobs independently, for example:
bash runs/m60_6_calibration_prep/job_scripts/scenario_000_json.sh

PYTHONPATH=cli:. python3 -m utilities.ordo_pathwalk.cli dry-run-collect   --plan runs/m60_6_calibration_prep/DRY_RUN_PLAN.json
```
...
```text
utilities/ordo_pathwalk/MODEL_BENCHMARK_PROTOCOL.md
```
...
```bash
PYTHONPATH=cli:. python3 -m utilities.ordo_pathwalk.cli model-benchmark-pilot \
  --out runs/m60_6_4_transcript_replay_pilot \
  --scenario-count 3 \
  --runtime-view json \
  --force

PYTHONPATH=cli:. python3 -m utilities.ordo_pathwalk.cli model-benchmark-collect \
