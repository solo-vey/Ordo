...
Usage:
    python3 -m utilities.ordo_pathwalk.cli init-tree --depth 6 --branching 2 6 --seed 42 --out DIR
    python3 -m utilities.ordo_pathwalk.cli gen-scenarios --tree DIR --count 10 --seed 100 --out DIR
    python3 -m utilities.ordo_pathwalk.cli run --tree DIR --scenario SCRIPT.json --driver anthropic:claude-sonnet-5 --cli-mode enforced --runtime-view ordo-code --out DIR
    python3 -m utilities.ordo_pathwalk.cli score --tree DIR --scenario SCRIPT.json --sandbox SANDBOX_DIR --transcript TRANSCRIPT.json
    python3 -m utilities.ordo_pathwalk.cli self-feed-next --session DIR
    python3 -m utilities.ordo_pathwalk.cli self-submit --session DIR --turn-index N --response-file PATH [--asked-node ID]
"""
