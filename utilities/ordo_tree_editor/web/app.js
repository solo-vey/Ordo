...
function entryNodeId() { return state.source?.graph_contract?.entry_node || state.source?.playbook?.entry_node || state.graph?.nodes?.[0]?.id; }
...
function render() {
  ...
  (state.graph.nodes || []).forEach(node => {
    ...
    const element = document.createElement("article"); element.className = `node ${node.terminal ? "terminal" : ""} ${state.selected === node.id ? "selected" : ""} ${state.pendingTransitionSource === node.id ? "transition-source" : ""}`;
    element.style.left = `${pos.x}px`; element.style.top = `${pos.y}px`; element.dataset.id = node.id;
    element.innerHTML = `<div class="node-id">${node.id}</div><div class="node-label"></div><div class="node-type">${node.answer_type}</div><div class="node-toolbar"><button type="button">Add transition</button></div>`;
...
function showEdgeMenu(edge, event) {
  if (edge.storage !== "transitions") return;
...
async function deleteDirectTransition(edge) { const source = state.source.nodes.find(node => node.id === edge.source); if (!source || !source.transitions || source.transitions[edge.key] !== edge.target) return alert("The selected transition is no longer available."); if (!confirm(`Delete transition ${edge.source} → ${edge.target}?`)) return; delete source.transitions[edge.key]; state.selectedEdge = null; hideEdgeMenu(); await refresh(state.source); }
function renderTransitionManager(node) { const manager = document.querySelector("#node-transition-manager"), list = document.querySelector("#outgoing-transition-list"); manager.hidden = !node; if (!node) return; list.innerHTML = ""; const direct = Object.entries(node.transitions || {}).filter(([, target]) => typeof target === "string" && !target.startsWith("$")); if (!direct.length) list.textContent = "No direct transitions declared for this node."; direct.forEach(([key, target]) => { const row = document.createElement("div"); row.className = "transition-row"; const text = document.createElement("span"); text.textContent = `${key} → ${target}`; const remove = document.createElement("button"); remove.type = "button"; remove.className = "danger"; remove.textContent = "Delete"; remove.addEventListener("click", () => deleteDirectTransition({ source: node.id, target, storage: "transitions", key })); row.append(text, remove); list.append(row); }); if (node.on_answer) { const note = document.createElement("p"); note.className = "hint"; note.textContent = "Nested answer transitions are managed in the Answer transitions (on_answer) section below."; list.append(note); } }
function renderInspector() { const transitionPanel = document.querySelector("#transition-inspector"); transitionPanel.hidden = !state.selectedEdge; if (state.selectedEdge) { const edge = state.selectedEdge; document.querySelector("#transition-summary").textContent = `${edge.source} → ${edge.target} (${edge.key})`; const removable = edge.storage === "transitions"; document.querySelector("#delete-transition").hidden = !removable; document.querySelector("#transition-note").textContent = removable ? "This direct transition can be deleted." : "This is a nested on_answer transition. Edit the Answer transitions (on_answer) node section to change it."; } const node = selectedNode(); document.querySelector("#selection-help").hidden = Boolean(node); form.hidden = !node; renderTransitionManager(node); if (!node) return; const view = state.graph.nodes.find(item => item.id === node.id); document.querySelector("#node-summary").textContent = `${node.id} · ${view?.answer_type || node.kind || "unspecified"}`; const container = document.querySelector("#node-sections"); container.innerHTML = ""; const sections = view?.sections || []; const ordered = [...sections].sort((a, b) => (sectionLabels[a.key] ? 0 : 1) - (sectionLabels[b.key] ? 0 : 1)); ordered.forEach(section => { const labels = sectionLabels[section.key] || ["Additional field", section.key]; const wrapper = document.createElement("div"); wrapper.className = "node-section"; const label = document.createElement("label"); label.textContent = `${labels[0]} (${labels[1]})`; const field = document.createElement("textarea"); field.dataset.sectionKey = section.key; field.dataset.originalValue = section.value_yaml; field.rows = Math.max(2, Math.min(12, section.value_yaml.split("\n").length + 1)); field.value = section.value_yaml; field.spellcheck = false; field.addEventListener("input", updateYamlPreview); label.append(field); wrapper.append(label); container.append(wrapper); }); showInspectorTab(state.inspectorTab); }
...
async function createTransition(sourceId, targetId) {
  ...
  const source = state.source.nodes.find(node => node.id === sourceId), target = state.source.nodes.find(node => node.id === targetId);
  ...
  if (direct) {
    ...
    target.allowed_from ||= [];
    if (!target.allowed_from.includes(sourceId)) target.allowed_from.push(sourceId);
  }
...
form.addEventListener("submit", async event => { event.preventDefault(); if (!state.selected) return; const sections = Object.fromEntries([...document.querySelectorAll("[data-section-key]")].map(field => [field.dataset.sectionKey, field.value])); try { const data = await request("/api/update-node-sections", { source: state.source, old_id: state.selected, sections }); state.source = data.source; state.graph = data.graph; state.positions = {}; state.manualPositions = new Set(); state.selected = data.node_id; render(); } catch (error) { alert(error.message); } });
document.querySelector("#delete-node").addEventListener("click", async () => { if (!state.selected) return; const connected = (state.graph.edges || []).filter(edge => edge.source === state.selected || edge.target === state.selected); if (connected.length) return alert("This node still has transitions. Update or remove those transitions before deleting it."); if (!confirm(`Delete ${state.selected}?`)) return; state.source.nodes = state.source.nodes.filter(n => n.id !== state.selected); state.positions = {}; state.manualPositions = new Set(); await refresh(state.source); });
document.querySelector("#delete-transition").addEventListener("click", () => { if (state.selectedEdge?.storage === "transitions") deleteDirectTransition(state.selectedEdge); });
document.querySelector("#cancel-transition-mode").addEventListener("click", () => { state.pendingTransitionSource = null; render(); });
document.querySelector("#edge-delete-action").addEventListener("click", () => { if (state.selectedEdge?.storage === "transitions") deleteDirectTransition(state.selectedEdge); });
