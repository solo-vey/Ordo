...
def resolve_target(g: Graph, target: str, location: str, external_terminals: set[str] | None = None) -> str:
    tid = target_to_node_id(target)
    ...
    if external_terminals and target in external_terminals:
...
def parse_nodes(data: Mapping[str, Any], input_path: Path, artifact_mode: str) -> Graph:
    ...
    graph_contract = data.get("graph_contract") or {}
    external_terminals = set(graph_contract.get("external_terminal_targets", []) or []) if isinstance(graph_contract, Mapping) else set()
    ...
    for index, node in enumerate(nodes):
        ...
        if "next" in on_answer and isinstance(on_answer.get("next"), str):
            ...
            tgt = resolve_target(g, target, f"nodes[{index}].on_answer.next", external_terminals)
        ...
        for ans, spec in on_answer.items():
            ...
            tgt = resolve_target(g, target, f"nodes[{index}].on_answer.{ans}", external_terminals)
...
def collect_state_field_sources(data: Mapping[str, Any]) -> dict[str, set[str]]:
    """Map state fields to nodes that write them via update_state."""
    field_nodes: dict[str, set[str]] = defaultdict(set)
    ...
    for node in nodes:
        ...
        if isinstance(on_answer.get("update_state"), Mapping):
            for key in on_answer["update_state"].keys():
                field_nodes[str(key)].add(node_id)

        for answer_key, spec in on_answer.items():
            if answer_key in {"update_state", "next", "artifacts", "output_artifacts"}:
                continue
            if isinstance(spec, Mapping) and isinstance(spec.get("update_state"), Mapping):
                for key in spec["update_state"].keys():
                    field_nodes[str(key)].add(node_id)

    return field_nodes


def gate_label(gate: Mapping[str, Any]) -> str:
    gid = str(gate.get("id") or "GATE")
    condition = str(gate.get("condition") or "").strip()
    if condition:
        if len(condition) > 72:
...
def attach_gates_to_graph(g: Graph, data: Mapping[str, Any]) -> Graph:
    """Attach gates contextually.

    Rules:
    - if a gate can be linked to one or more workflow nodes by matching `state.<field>`
      in the gate condition against node update_state writes, connect node -> gate;
    - else if the gate is referenced by outputs.allowed_after, connect gate -> output
      and let it live near the output/package node;
    - gates with neither node attachment nor output attachment stay unconnected and
      can be visually grouped by Graphviz on the side.
    """
    gates = data.get("gates") or []
    ...
    for gate in gates:
        if isinstance(gate, Mapping) and gate.get("id"):
            gate_map[str(gate["id"])] = gate

    if not gate_map:
        return g

    # Outputs as package/archive containers are needed as anchor targets for some gates.
    output_ids: list[str] = []
    ...
    field_nodes = collect_state_field_sources(data)
    node_to_gates: dict[str, set[str]] = defaultdict(set)
    connected_gates: set[str] = set()

    # 1) attach by direct transition target if already used in workflow
    for edge in list(g.edges):
        if edge.target in g.nodes and g.nodes[edge.target].type == "gate":
            connected_gates.add(edge.target)

    # 2) attach by state field matches from condition
    for gate_id, gate in gate_map.items():
        gid = safe_id(gate_id)
        if gid not in g.nodes:
            add_node_once(g, GNode(gid, "gate", gate_label(gate), gate, True))
        else:
            # upgrade label from plain "Gate" to more descriptive one
            old = g.nodes[gid]
            g.nodes[gid] = GNode(old.id, old.type, gate_label(gate), gate, old.synthetic)

        condition = str(gate.get("condition") or "")
        fields = re.findall(r"state\.([A-Za-z0-9_]+)", condition)
        matched_nodes: set[str] = set()

        for field in fields:
            matched_nodes |= field_nodes.get(field, set())

        # explicit special-case: approval gate is usually confirmed at review approval node
        if gate_id == "G_DRAFT_REVIEW_APPROVED" and "N16_DRAFT_REVIEW_APPROVAL" in g.nodes:
            matched_nodes.add("N16_DRAFT_REVIEW_APPROVAL")

        for nid in sorted(matched_nodes):
            if nid in g.nodes:
                edge_exists = any(e.source == nid and e.target == gid for e in g.edges)
                if not edge_exists:
                    g.edges.append(Edge(nid, gid, "satisfies", "dashed"))
                connected_gates.add(gid)
                node_to_gates[nid].add(gid)

    # 3) attach to outputs.allowed_after
    for output in outputs if isinstance(outputs, list) else []:
        raw_id, _ = output_label(output)
        oid = "output_" + safe_id(raw_id)
        allowed_after = output.get("allowed_after") if isinstance(output, Mapping) else None
        if isinstance(allowed_after, list):
            for gate_ref in allowed_after:
                gid = safe_id(str(gate_ref))
                if gid in g.nodes:
                    if not any(e.source == gid and e.target == oid for e in g.edges):
                        g.edges.append(Edge(gid, oid, "allowed_after", "dashed"))
                    connected_gates.add(gid)

    # 4) mark hanging gates by keeping them present but unconnected if nothing matched.
    # Graphviz will place them aside. Nothing more is required here.

    return g


def gate_groups_for_dot(g: Graph) -> tuple[list[str], list[str], list[str]]:
    """Return node ids for gates attached to workflow nodes, attached to outputs only, and hanging."""
    gate_ids = [nid for nid, node in g.nodes.items() if node.type == "gate" or node.type.endswith("_focus") and node.type.startswith("gate")]
    output_ids = {nid for nid, node in g.nodes.items() if node.type == "output"}

    connected_sources: dict[str, set[str]] = defaultdict(set)
    connected_targets: dict[str, set[str]] = defaultdict(set)
    for edge in g.edges:
        connected_sources[edge.source].add(edge.target)
        connected_targets[edge.target].add(edge.source)
    ...
    for gid in gate_ids:
        sources = connected_targets.get(gid, set())  # nodes pointing into gate
        targets = connected_sources.get(gid, set())  # gate pointing out
        has_node_source = any(src in g.nodes and g.nodes[src].type not in {"gate", "output", "artifact"} for src in sources)
        has_output_target = any(tgt in output_ids for tgt in targets)

        if has_node_source:
            node_attached.append(gid)
        elif has_output_target:
            output_attached.append(gid)
        else:
            hanging.append(gid)

    return node_attached, output_attached, hanging
...
def write_dot(g: Graph, out: Path) -> None:
    node_attached_gates, output_attached_gates, hanging_gates = gate_groups_for_dot(g)
    ...
    # Optional side cluster for hanging gates.
    if hanging_gates:
        lines.append('  subgraph cluster_hanging_gates {')
        lines.append('    label="Unattached / policy gates";')
        lines.append('    color="#FBCFE8";')
        lines.append('    style="rounded,dashed";')
        for node_id in hanging_gates:
            node = g.nodes[node_id]
    ...
    # Keep output-attached gates near outputs.
    for gid in output_attached_gates:
        lines.append(f"  {{ rank=same; {gid}; }}")
...
def build_parser() -> argparse.ArgumentParser:
  ...
  Include gates contextually:
    python ordo_graph.py input.ordo.yaml --include-gates --format svg --out full_with_gates.svg
...
""",
    ...
    parser.add_argument("--gate-mode", choices=["referenced", "all", "none"], default="referenced", help="Basic gate rendering filter. Default: referenced.")
    parser.add_argument("--include-gates", action="store_true", help="Include gates contextually in the graph: attach to workflow nodes when possible, otherwise to outputs, otherwise keep hanging gates in a side block.")
    return parser
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        if not graph.has_errors and args.gate_mode == "none":
            # remove all gates
            graph.nodes = {nid: node for nid, node in graph.nodes.items() if node.type != "gate"}
            graph.order = [nid for nid in graph.order if nid in graph.nodes]
            graph.edges = [e for e in graph.edges if e.source in graph.nodes and e.target in graph.nodes]

        if not graph.has_errors and args.include_gates:
            graph = attach_gates_to_graph(graph, data)
