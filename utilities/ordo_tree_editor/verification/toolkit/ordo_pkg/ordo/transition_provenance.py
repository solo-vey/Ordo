...
def validate_transition_provenance(source: dict[str, Any]) -> dict[str, Any]:
    ...
    outbound: dict[str, set[str]] = {
        vertex_id: {target for target in targets if target in ids}
    ...
    for node_id, declared in inbound.items():
        kind = "gate" if node_id in gate_ids else "node"
        for pred in sorted(declared):
            if pred not in ids:
                issues.append(TransitionProvenanceIssue("error", "GRAPH_INBOUND_SOURCE_MISSING", f"Inbound predecessor {pred!r} does not exist.", f"{kind}s[{node_id}].allowed_from", pred, node_id, "inbound"))
            elif node_id not in outbound.get(pred, set()):
                issues.append(TransitionProvenanceIssue("error", "GRAPH_INBOUND_WITHOUT_OUTBOUND", f"Vertex {node_id} accepts {pred}, but {pred} has no direct outbound edge to {node_id}.", f"{kind}s[{node_id}].allowed_from", pred, node_id, "inbound"))
    ...
    for source_id, targets in outbound.items():
        for target_id in sorted(targets):
            ...
            if source_id not in inbound.get(target_id, set()):
                target_kind = "gate" if target_id in gate_ids else "node"
                issues.append(TransitionProvenanceIssue("error" if strict else "warning", "GRAPH_OUTBOUND_NOT_ACCEPTED", f"Direct edge {source_id} -> {target_id} is not accepted by target allowed_from.", f"{target_kind}s[{target_id}].allowed_from", source_id, target_id, "outbound"))
    ...
    for node_id, node in by_id.items():
        ...
        if node_id == entry:
            ...
            if "root" not in modes:
                issues.append(TransitionProvenanceIssue("error", "GRAPH_ROOT_ENTRY_MODE_MISSING", "Entry vertex must declare entry_modes: [root] (additional explicit modes are allowed).", f"{kind}s[{node_id}].entry_modes", None, node_id, "entry"))
        elif strict and not inbound.get(node_id) and not set(node.get("entry_modes") or []):
            issues.append(TransitionProvenanceIssue("error", "GRAPH_INBOUND_DECLARATION_MISSING", f"Non-entry {kind} {node_id} must declare a direct predecessor or an explicit entry mode.", f"{kind}s[{node_id}].allowed_from", None, node_id, "inbound"))
...
def validate_node_entry(source: dict[str, Any], *, target_node_id: str, previous_node_id: str | None, entry_mode: str = "transition") -> dict[str, Any]:
    ...
    target=graph_topology(source)["by_id"].get(target_node_id)
    if target is None:
